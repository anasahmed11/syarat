        ...
        <div>
            ...
            <form>
                <div class="row">
                    ...
                    <div class="col-md-2">
                        {{Form::text('department_name',null, array('class' => 'form-control'))}}
                        <br>
                    </div>
                    <div class="col-md-2">
                        <button class="btn btn-dark" type="submit"><i class="fas fa-search"></i> search</button>
                        <button class="btn btn-danger "><a href="{{route('departments')}}" style="color: white"><i
                    ...
                    <table class="article-table table table-striped  ">
                        ...
                        @foreach($data as $row)
                            <tr class="article-{{$row->id}}">
                                ...
                                <td>{{$row->employees_salaries}}</td>
                                <td>
                                    <button class="edit-article btn btn-info" data-toggle="modal"
                                            data-target="#edit-modal-article" data-id="{{ $row->id }}"
                                            data-name="{{$row->name}}">update
                                    </button>
                                </td>
                                <td>
                                    <button class="delete-article btn btn-danger" data-id="{{$row->id}}">delete</button>
                                </td>
                            </tr>
                        @endforeach
                        {{ $data->links() }}
                ...
                <div class="modal-dialog modal-dialog-centered" style="max-width:800px;" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            {{Form::open(array('id'=>'add-data-form','enctype'=>'multipart/form-data'))}}
                            {{Form::label('name', 'name')}}
                            {{Form::text('name','',['class' => 'form-control'])}}
                            <br><br>
                            {{Form::submit('save',['class' => 'btn btn-dark btn-lg btn-block','id'=>'add-data'])}}
                            {{ Form::close() }}
                            <br>
                        </div>
